...
def main():
    ...
    train_dir = 'D:/DATA/IXI_brain/network_training/Train/'
    ...
    model = TransMorph.TransMorphCascadeAd(config, SVF=True)
    model.cuda()
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            reg_code = torch.clamp(reg_code, min=1e-4, max=1.)
            smo_weight = reg_code * weights[1]
            flow, wts = model((x_half,y_half), reg_code)
            ...
            loss_reg2 = criterion_reg2(flow, wts) * weights[2]
            ...
            reg_code = torch.clamp(reg_code, min=1e-4, max=1.)
            smo_weight = reg_code * weights[1]
            flow, wts = model((y_half, x_half), reg_code)
            flow = F.interpolate(flow.cuda(), scale_factor=2, mode='trilinear', align_corners=False) * 2
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                reg_code = torch.tensor([1.4/weights[2]], dtype=x.dtype, device=x.device).unsqueeze(dim=0)
