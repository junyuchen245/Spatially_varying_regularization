...
class OMT(torch.nn.Module):
    def __init__(self, sigmas):
        super(OMT, self).__init__()
        self.sigmas = np.abs(np.log(sigmas[-1]/sigmas))
        self.normalize = np.abs(np.log(sigmas[-1]/sigmas[0]))

    def forward(self, weights):
        OMT = 0
        for i in range(len(self.sigmas)):
            OMT = OMT+weights[:, i:i+1]*self.sigmas[i]
        OMT = OMT/self.normalize
        return OMT.mean()

class OMTReg(torch.nn.Module):
    def __init__(self, eps=1e-6):
        super(OMTReg, self).__init__()
        self.eps = eps
        self.normalize = np.abs(np.log(eps))

    def forward(self, weights):
        OMT = torch.abs(torch.log(torch.clamp(weights, self.eps, 1.0)))
        OMT = OMT/self.normalize
        return OMT.mean()
