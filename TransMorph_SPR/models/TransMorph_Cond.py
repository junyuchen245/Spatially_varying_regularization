...
class ConditionalLayerNorm(nn.Module):
    ...
    def forward(self, input, latent_code):
        ...
        style = self.style(latent_code).unsqueeze(dim=1)
...
class HyperSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
        ...
        self.cd_norm1 = ConditionalLayerNorm(dim, dim)
        self.cd_norm2 = ConditionalLayerNorm(dim, dim)
    ...
    def forward(self, x, mask_matrix, latent_code):
        H, W, T = self.H, self.W, self.T
        B, L, C = x.shape
        assert L == H * W * T, "input feature has wrong size"

        latent_feat = self.mapping(latent_code)

        shortcut = x.clone()

        x = self.cd_norm1(x, latent_feat)
        x = x.view(B, H, W, T, C)
        ...
        x = x + self.drop_path(self.mlp(self.cd_norm2(x, latent_feat)))
        return x
...
class HyperPatchMerging(nn.Module):
    r""" Patch Merging Layer.
    Args:
        dim (int): Number of input channels.
        norm_layer (nn.Module, optional): Normalization layer.  Default: nn.LayerNorm
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, reduce_factor=2):
        ...
        self.dim = dim
        self.reduction = nn.Linear(8 * dim, (8//reduce_factor) * dim, bias=False)
        self.norm = ConditionalLayerNorm(8 * dim, 8 * dim)
        self.mapping = nn.Sequential(
            nn.Linear(1, 8 * dim),
            nn.LeakyReLU(0.2),
    ...
    def forward(self, x, H, W, T, latent_code):
        """
        x: B, H*W*T, C
        """
        latent_feat = self.mapping(latent_code)
    ...
    def forward(self, x, H, W, T, latent_code):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, attn_mask, latent_code)
            else:
                x = blk(x, attn_mask, latent_code)
        if self.downsample is not None:
            x_down = self.downsample(x, H, W, T, latent_code)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        for i_layer in range(self.num_layers):
            layer = HyperLayer(dim=int(embed_dim * 2 ** i_layer),
                                ...
                                norm_layer=norm_layer,
                                downsample=HyperPatchMerging if (i_layer < self.num_layers - 1) else None,
    ...
    def forward(self, x, latent_code):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, T, x, Wh, Ww, Wt = layer(x, Wh, Ww, Wt, latent_code)
...
class CdConv3dReLU(nn.Module):
    def __init__(
        ...
        super(CdConv3dReLU, self).__init__()
        self.conv = nn.Conv3d(
            in_channels,
            out_channels,
            kernel_size,
        ...
        self.nm = ConditionalInstanceNorm3D(out_channels, out_channels)

    def forward(self, x, latent_code):
        latent_feat = self.mapping(latent_code)
        x = self.conv(x)
        x = self.nm(x, latent_feat)
        x = self.relu(x)
        return x
...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        self.c1 = CdConv3dReLU(2, embed_dim//2, 3, 1)
    ...
    def forward(self, x, latent_code):
        ...
        if self.if_convskip:
            ...
            f4 = self.c1(x_s1, latent_code)
        else:
            f4 = None

        out_feats = self.transformer(x, latent_code)
        ...
        flow = self.tri_up(flow)
        out = self.spatial_trans(source, flow)
        return out, flow

class TransMorphCascadeAd(nn.Module):
    def __init__(self, config, time_steps=7, SVF=False):
        '''
        ...
        for t in range(self.time_steps):
            self.cs.append(CdConv3dReLU(2, embed_dim // 2, 3, 1))
        ...
        self.sigma_0 = CdConv3dReLU(config.reg_head_chan, embed_dim // 2, 3, 3//2)
        self.sigma_1 = CdConv3dReLU(embed_dim // 2, embed_dim // 2, 3, 3 // 2)
        ...
        self.SVF = SVF
        if self.SVF:
            self.vec_int = VecInt(config.img_size, 7)
        self.wts_act = nn.Sigmoid()
        self.eps = 1e-6

    def forward(self, inputs, latent_code):
        mov, fix = inputs
        x_cat = torch.cat((mov, fix), dim=1)
        x_s1 = self.avg_pool(x_cat)
        out_feats = self.transformer(x_cat, latent_code)  # (B, n_patch, hidden)
        if self.if_convskip:
            f3 = self.c1(x_s1, latent_code)
        else:
            f3 = None
        if self.if_transskip:
            f1 = out_feats[-2]
        ...
        xs = 0
        for t in range(self.time_steps):
            f_out = self.cs[t](torch.cat((def_x, fix), dim=1), latent_code)
        ...
        x_weight = self.sigma_0(xs, latent_code)
        x_weight = self.sigma_1(x_weight, latent_code)
        x_weight = self.upsamp(self.sigma_2(x_weight))
